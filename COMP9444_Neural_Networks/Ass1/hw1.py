...
def onelayer(X, Y, layersize=10):
    ...
    pixels = X.get_shape().as_list()[1]

    w = tf.Variable(tf.zeros([pixels, layersize]))
...
def twolayer(X, Y, hiddensize=30, outputsize=10):
    ...
    # Initialised to small random slightly positive weights
    w1 = weight_variable([pixels, hiddensize])
    b1 = bias_variable([hiddensize])
    h1_b = tf.nn.relu(tf.matmul(X, w1) + b1)

    # Second layer with Softmax Activation
    w2 = tf.Variable(tf.zeros([hiddensize, outputsize]))
    b2 = tf.Variable(tf.zeros([outputsize]))

    logits = tf.matmul(h1_b, w2) + b2
    preds = tf.nn.softmax(logits)
...
def convnet(X, Y, convlayer_sizes=[10, 10], \
    ...
    # First convolutional layer. Grey so 1 dimension for color.
    w1 = weight_variable(filter_shape + [1] + [convlayer_sizes[0]])
    b1 = bias_variable([convlayer_sizes[0]])
    conv1 = conv2d(X, w1) + b1
    h_conv1 = tf.nn.relu(conv1)

  # Second convolutional layer
    w2 = weight_variable(filter_shape + [convlayer_sizes[0]] + [convlayer_sizes[1]])
    b2 = bias_variable([convlayer_sizes[1]])
    conv2 = conv2d(h_conv1, w2) + b2
    h_conv2 = tf.nn.relu(conv2)

  # Final fully connected later
    conv2_flat = tf.reshape(h_conv2, [128, 28 * 28 * 10])
    w, b, logits, preds, batch_xentropy, batch_loss = onelayer(conv2_flat, Y, layersize=10)
...
# Helper functions - from https://www.tensorflow.org/get_started/mnist/pros
